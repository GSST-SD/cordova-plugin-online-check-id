...
- (void)getMessageCallBack:(NSString*)_message;
...
- (void)getMessage:(CDVInvokedUrlCommand *)command{
    ...
    NSLog(@"myPeripherali other: %@, macId: %@, %@", other, macId, curConnectPeripheral);
    
    if(curConnectPeripheral != nil){
        //        if(curConnectPeripheral != nil && [macId isEqualToString:other]){
        isexit = YES;
        if(self.linkedPeripheral == nil){
            [self getMessageCallBack:@"请先选中要连接的蓝牙设备!"];
            NSLog(@"请先选中要连接的蓝牙设备!");
        }else{
            if(self.linkedPeripheral.peripheral.state != CBPeripheralStateConnected){
                NSLog(@"蓝牙处于未连接状态,先连接蓝牙!");
                [self getMessageCallBack:@"蓝牙处于未连接状态,先连接蓝牙!"];
            }else{
...
-(void)getMessageCallBack:(NSString*)message {
    CDVPluginResult *pluginResult = nil;
    NSString *_message = message;
    NSDictionary * IDCardInfo = [NSDictionary dictionaryWithObjectsAndKeys:
                                 @NO, @"status",
                                 @"", @"data",
...
- (void)connect:(CDVInvokedUrlCommand *)command{
    NSLog(@"开始连接蓝牙%@", self.linkedPeripheral);
    if (self.linkedPeripheral == nil){
